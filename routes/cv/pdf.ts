...
export const handler: Handlers = {
  async GET(_req: Request, _ctx: FreshContext) {
    try {
      ...
      const addText = (text: string, x: number, y: number, maxWidth: number, fontSize = 9, fontStyle = 'normal', color = [0, 0, 0]) => {
      ...
      yPosition = addText(typedCvData.profile.name, margin, yPosition, contentWidth, 18, 'bold');
      yPosition = addText(typedCvData.profile.title, margin, yPosition + 1, contentWidth, 11, 'normal', [60, 60, 60]);
      ...
      contactInfo.forEach(line => {
        yPosition = addText(line, margin, yPosition, contentWidth, 9, 'normal', [60, 60, 60]);
      });
      ...
      yPosition = addText(statsText, margin, yPosition, contentWidth, 9, 'normal', [100, 100, 100]);
      yPosition += 5;

      // Professional Summary Section
      yPosition = addSectionHeader("Professional Summary", yPosition);
      yPosition = addText(typedCvData.professionalSummary.description, margin, yPosition, contentWidth, 9, 'normal', [40, 40, 40]);
      yPosition += 4;
      ...
      yPosition = addText(coreExpertiseText, margin, yPosition, contentWidth, 8, 'normal', [40, 40, 40]);
      yPosition += 3;

      // Tech Stack in compact format
      Object.entries(typedCvData.skills.techStack).forEach(([category, technologies]) => {
        checkNewPage(8);
        yPosition = addText(`${category}: ${technologies.join(", ")}`, margin, yPosition, contentWidth, 8, 'normal', [60, 60, 60]);
        yPosition += 1;
      });
      ...
      typedCvData.experience.forEach((job) => {
        ...
        // Job header
        yPosition = addText(job.title, margin, yPosition, contentWidth - 40, 10, 'bold');
        
        // Company name with link and period
        const companyText = `${job.company} | ${job.period}`;
        yPosition = addText(companyText, margin, yPosition, contentWidth, 10, 'normal', [247, 147, 26]);
        
        // Add clickable link for company if URL exists
        if (job.companyUrl) {
          const companyTextWidth = doc.getTextWidth(job.company);
          doc.link(margin, yPosition - 6, companyTextWidth, 3, { url: job.companyUrl });
        }
        ...
        job.achievements.forEach((achievement) => {
          checkNewPage(5);
          yPosition = addText(`• ${achievement}`, margin, yPosition, contentWidth, 9, 'normal', [40, 40, 40]);
        });
        
        // Add technologies if they exist - more compact
        if (job.technologies) {
          yPosition += 1;
          yPosition = addText(`Technologies: ${job.technologies.join(", ")}`, margin, yPosition, contentWidth, 8, 'normal', [100, 100, 100]);
        }
       ...
       typedCvData.personalProjects.forEach((project) => {
         ...
         // Project header
         yPosition = addText(project.name, margin, yPosition, contentWidth - 30, 10, 'bold');
         
         // Add clickable project link in orange
         yPosition = addText(project.url, margin, yPosition, contentWidth, 8, 'normal', [247, 147, 26]);
         const linkWidth = doc.getTextWidth(project.url);
         doc.link(margin, yPosition - 5, linkWidth, 3, { url: project.url });
         yPosition += 1;
         
         // Add description
         yPosition = addText(project.description, margin, yPosition, contentWidth, 9, 'normal');
         yPosition += 1;
         
         yPosition = addText(`Technologies: ${project.technologies.join(", ")}`, margin, yPosition, contentWidth, 8, 'normal', [100, 100, 100]);
      ...
      typedCvData.education.forEach((edu, index) => {
        ...
        yPosition = addText(edu.degree, margin, yPosition, contentWidth - 30, 10, 'bold');
        yPosition = addText(`${edu.institution} | ${edu.period}`, margin, yPosition, contentWidth, 9, 'normal', [247, 147, 26]);
        yPosition += 1;
        
        edu.details.forEach((detail) => {
          yPosition = addText(`• ${detail}`, margin, yPosition, contentWidth, 9, 'normal', [40, 40, 40]);
        });
